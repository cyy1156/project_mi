...
from ea import prepare_ea_eval_batch
from infer import load_ft_fold
from otta_infer import build_eval_tensors
from ref_cov import load_ref_cov_src
from adabn import freeze_all_params, restore_bn_running, snapshot_bn_running, stream_predict_adabn
...
def _predict_trial_windows(
    ...
    sp = stream_predict_adabn(model, X, device, update_bn=True, conf_tau=None)
...
def run_c1_subject(
    ...
    *,
    ...
    pack = build_eval_tensors(stream, split, ea_ref="src", r_ref_src=r_ref_src)
    ...
    if np.isfinite(thirds["first"]) and np.isfinite(thirds["last"]):
        if thirds["last"] < thirds["first"] - 0.03:
            collapse = True
    a0 = ANCHOR_A0.get(head, (float("nan"),))[0]
    if head == "three" and acc < a0:
        collapse = True
