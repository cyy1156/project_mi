...
def eval_one_arm(
    ...
    *,
    ...
    md = [
        ...
        f"- EA={arm.ea_ref or 'off'} AdaBN={arm.adabn} gate={arm.gate} τ={tau}",
        "",
    ]
    ...
    delta_block = {}
    if anchor and "three" in macro:
        a_mean = anchor["three"][0]
        d = macro["three"]["mean"] - a_mean
        delta_block["three_vs_anchor"] = d
        n_pass = sum(1 for v in macro["three"]["per_subject"] if v - a_mean >= 0.02)
        try:
            p = float(wilcoxon(macro["three"]["per_subject"]).pvalue)
        except Exception:
            p = float("nan")
        delta_block.update(
            {"n_pass_ge_0.02": n_pass, "wilcoxon_p": p, "anchor_three": a_mean}
        )
    ...
    if macro:
        ...
        if delta_block:
            md += [
                "",
                f"- ΔThree vs 锚点: {delta_block.get('three_vs_anchor', float('nan')):+.4f}",
                f"- 达标数(≥+0.02): {delta_block.get('n_pass_ge_0.02', '—')}",
                f"- Wilcoxon p: {delta_block.get('wilcoxon_p', float('nan')):.4g}",
            ]
        md.append("")
