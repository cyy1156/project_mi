"""Three 下游（LeJEPA · BCI2a 4s）：J0 / J2 / J2_random / J3。

早停与选优：**Val balanced_accuracy**（方案 12）；主报仍登记 Acc_paper + BalAcc。
...
def train_one_fold(
    *,
    ...
    best_val_acc_paper = float("nan")
    bad = 0
    fold_dir = out_dir / f"fold{fold}"
    fold_dir.mkdir(parents=True, exist_ok=True)
    early_key = getattr(hp, "early_stop", "balanced_accuracy")
    ...
    for ep in range(1, hp.max_epochs + 1):
        ...
        val_ap = float(val_trial["acc_paper"])
        val_bal = float(val_trial["balanced_accuracy"])
        win_bal = float(val_win["balanced_accuracy"])
        if early_key == "acc_paper":
            score = val_ap
        else:
            score = val_bal  # 默认 balanced_accuracy
        print(
            f"fold{fold} ep {ep:03d}  tr={total/max(n,1):.4f}  "
            f"val_BalAcc={val_bal:.4f}  val_AccPaper={val_ap:.4f}  "
            f"win_BalAcc={win_bal:.4f}",
            flush=True,
        )
        if score > best_score:
            best_score = score
            best_val_acc_paper = val_ap
            best_ep = ep
            ...
            torch.save(
                {
                    ...
                    "fold": fold,
                    "epoch": ep,
                    "early_stop": early_key,
                    "val_balanced_accuracy": val_bal,
                    "val_acc_paper": val_ap,
                    "model": best_state,
                },
            ...
            bad += 1
            if bad >= hp.patience:
                print(f"  early stop @ ep {ep} (monitor={early_key})", flush=True)
                break
    ...
    return {
        ...
        "best_val_acc_paper": best_val_acc_paper,
...
def main():
    ...
    p.add_argument("--j1-dir", type=str, default="", help="含 fold*_lejepa.pt 的 j1 目录")
    ...
    print(
        f"[{args.arm}] out={out_dir} device={device} "
        f"early_stop={hp.early_stop} data={hp.data_tag}",
        flush=True,
    )
    ...
    bal = [float(r["test_trial_metrics"]["balanced_accuracy"]) for r in folds]
    f1 = [float(r["test_trial_metrics"]["f1_macro"]) for r in folds]
    summary = {
        "arm": args.arm,
        "data_tag": hp.data_tag,
        "early_stop": hp.early_stop,
        "note": "BCI2a 4s fixed window; do not compare Acc abs to OpenBMI shallow 0.540",
        "test_acc_paper_mean": float(np.mean(ap)),
        "test_acc_paper_std": float(np.std(ap)),
        "test_balanced_accuracy_mean": float(np.mean(bal)),
        "test_balanced_accuracy_std": float(np.std(bal)),
    ...
    print(
        ...
        f"±{summary['test_balanced_accuracy_std']:.4f}",
        flush=True,
    )
