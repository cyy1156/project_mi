"""S07-02 / S07-05：S3 init · Stieger cue 前半 FT · 后半评（含同后半零样本 Δ）。"""
...
RESULTS = RESULTS_ROOT / "S07-02_ft_half"
RESULTS_05 = RESULTS_ROOT / "S07-05_ft_half"
...
def main() -> None:
    p = argparse.ArgumentParser(description="S07-02/05 Stieger 前半 FT")
    ...
    md = [
        f"# S07-02/05 Stieger 前半 FT · {stamp}",
        "",
        f"- init：`{init_run}`",
    ...
    # S07-05 软链说明
    note = (
        f"# S07-05 ≡ S07-02（本套 init 已是 OpenBMI S3）\n\n"
        f"主记录见：`../S07-02_ft_half/{stamp}_shallow_ft_half/`\n"
        f"权重：`{weight_root}`\n"
    )
    (RESULTS_05 / f"{stamp}_see_S07-02.md").write_text(note, encoding="utf-8")
