...
class MaskFutureDualExpert(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.predictor_identity = bool(predictor_identity)
        self.expert_double = bool(expert_double)
        ...
        vp = int(vis_pts) if vis_pts is not None else (600 if n_times >= 1000 else n_times)
        self.i_vis, self.i_fut = segment_indices(n_times, vis_pts=vp, t_prime=self.t_prime)
        ...
        hid = 128 if expert_double else 64
        self.expert_cur = MLP([d, hid, n_outputs], dropout=0.0)
        if use_predictor and not predictor_identity:
    ...
    def forward(
        ...
        *,
        ...
        if (
            ...
            and (self.predictor is not None or self.predictor_identity)
            and self.i_fut
        ):
            if ema_target:
                if self.ema_encoder is None:
