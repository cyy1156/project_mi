"""对全部 1s 基线做一次前向冒烟（250 点 / bandpower / raw）。"""
...
def main() -> None:
    x_time = torch.randn(4, 8, 250)
    x_feat = torch.from_numpy(raw_to_bandpower(np.random.randn(4, 1, 8, 250).astype(np.float32)))
    x_raw = torch.from_numpy(squeeze_raw_1s(np.random.randn(4, 1, 8, 250).astype(np.float32)))
