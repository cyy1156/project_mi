"""Task + Three 五折：Val Acc_paper 早停；train batch balance；无 RAP；仅 T。"""
...
def _make_ds(X, y, input_kind: str, n_outputs: int):
    # time / raw：ArrayTask/Three（raw 已在 prepare_X 压成 (N,8,T)）
    if input_kind == "feat":
...
def _eval_split(
    ...
    *,
    ...
    Xs, ys = X[mask], y[mask]
    subs, tids = subjects[mask], trial_ids[mask]
    loader = DataLoader(
        _make_ds(Xs, ys, input_kind, n_classes),
...
def train_one_fold(
    ...
    *,
    ...
    train_loader = DataLoader(
        _make_ds(X[masks["train"]], y_tr, input_kind, n_outputs),
    ...
    model = build_model(8, int(X.shape[-1]), n_outputs, hp.drop_prob).to(device)
...
def run_baseline_main(
    *,
    ...
    p = argparse.ArgumentParser(
        description=f"{model_name} 2s/hop100 Acc_paper 重训（仅 T；balbatch）"
    )
    p.add_argument(
        "--data",
        default=SHARED.data_tag,
        choices=("bci2a_2s_hop100",),
        help="仅 BCI2a T（hop100）；禁用 E/Stieger",
    )
    ...
    out_name = f"{model_name}_2s_hop100_balbatch_accpaper"
    out_root = (
        TRAIN_LAB / "out" / "baseline_2s_hop100_accpaper" / out_name / data_tag / f"run_{stamp}"
    )
    ...
    append_md(
        ...
        "\n".join(
            [
                ...
                f"- device：`{device}`",
                f"- data：`{data_dir}`（**仅 BCI2a T / hop100**；不用 E）",
                f"- protocol：`{hp.protocol}` | early_stop=**Acc_paper** | **balbatch** | no_rap",
                f"- 读数口径：`Tw=2s hop=100ms bci2a_T_only early_stop=val_acc_paper select=test_acc_paper balbatch`",
