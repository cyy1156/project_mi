...
class IndexArrayDataset(Dataset):
    ...
    def __getitem__(self, i: int):
        ...
        x = self._X[idx].astype(np.float32, copy=True)
...
class PackedArrayDataset(Dataset):
    """折内打包窗：全量常驻内存（不再用 mmap）。"""

    def __init__(self, y_pack: np.ndarray, *, x_path: str | Path):
        self._X = np.load(str(x_path))  # 全量载入内存
        self.y = np.asarray(y_pack, dtype=np.int64)
        assert len(self.y) == len(self._X)

    def __len__(self) -> int:
        return len(self.y)

    def __getitem__(self, i: int):
        x = self._X[i].astype(np.float32, copy=True)
        if x.ndim == 3 and x.shape[0] == 1:
            x = x[0]
...
def _eval_one_pass(
    ...
    *,
    ...
    """单次遍历同时返回 (avg_loss, y_true, y_pred)，消除双重遍历。"""
    model.eval()
    total, n = 0.0, 0
    ys, ps = [], []
    ...
    for x, y in loader:
        ...
        if logits.ndim > 2:
            logits = logits.reshape(logits.shape[0], -1)
        loss = nn.functional.cross_entropy(logits, y_dev)
...
def _eval_split(
    ...
    *,
    ...
    # 单次遍历：同时拿到 loss 和预测，消除双重遍历
    va_loss, yt, yp = _eval_one_pass(
        model, loader, device,
...
def run_baseline_main(
    *,
    ...
    p.add_argument(
        ...
        choices=("openbmi_2s_fixed_cue2to4_noz",),
        help="仅 OpenBMI 固定窗无 z-score；禁用 BCI2a/Stieger 混训",
    )
    ...
    # 数据仅 ~250MB，全量载入内存，避免 mmap 逐条磁盘 I/O 导致 GPU 饥饿
    X = np.load(x_npy)
    ...
    if n_subjects < 3:
        raise RuntimeError(
            f"openbmi_2s_fixed_cue2to4_noz 当前仅 {n_subjects} 名被试（{data_dir}）。"
            "被试独立五折至少需要多名被试；请先跑："
            "python -m src.datasets.openbmi.batch_2s_fixed_cue2to4"
            "（或 --subjects 01,02,... 子集后再训）。"
        )
    x_path: str | None = None  # 数据已全量在内存，无需文件路径
    if prepare_X is not None:
        print(f"[load] prepare_X on X{tuple(X.shape)} …", flush=True)
        X = prepare_X(X)
        if hasattr(X, "filename"):  # prepare_X 返回 memmap → 拷入内存
            X = np.array(X)
    ...
    out_name = f"{model_name}_openbmi_2s_fixed_noz_balbatch_accpaper"
    out_root = (
        TRAIN_LAB
        / "out"
        / OUT_ROOT_TAG
