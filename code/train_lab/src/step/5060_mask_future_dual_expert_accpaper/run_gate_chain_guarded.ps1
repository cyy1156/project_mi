...
# 默认 GATE：A0_ref → A0 → A1 → P0 → A2 → P1 → P2（每臂 fold0）
...
$busy = @(Get-CimInstance Win32_Process -Filter "Name='python.exe'" -EA SilentlyContinue |
  Where-Object {
    $_.CommandLine -and (
      $_.CommandLine -match '5060_mask_future_dual_expert_accpaper\\run_arm\.py' -or
...
Write-Chain "START scheme17 $mode queue=$($queue -join ',') commit_limit=${cLimit}G free=$(Get-FreePhysGB)G timeout=${TimeoutSecPerArm}s batch=128/256 sigreg=1024"
if ($cLimit -lt 40) {
