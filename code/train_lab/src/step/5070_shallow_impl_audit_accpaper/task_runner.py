...
from shared_hparams import SHARED, SharedTrainHP, OUT_ROOT_TAG, SCHEME18_RUNS_TAG
...
def append_md(md_path: Path, text: str, out_root: Path, log_path: Path) -> None:
    ...
    latest.write_text(
        ...
        f"旁路方案 18：[`18_旁路_手写vs库_ShallowFBCSP_openbmi_accpaper/`](./18_旁路_手写vs库_ShallowFBCSP_openbmi_accpaper/)（**非正式十一模型表**）\n\n"
        f"- 本机（5070 · 方案18）本次记录：[`{rel}`](./{rel})\n"
        f"  - 权重：`{out_root}`\n"
        f"  - 日志：`{log_path}`\n",
        encoding="utf-8",
    )
