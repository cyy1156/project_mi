"""T 系列 v3 链：T1 → T1_128（无 T1_aux · 无 Cross-Attn / 无 Phase）。

用法：
  python chain_t_all.py
  python chain_t_all.py --from T1_128
  python chain_t_all.py --max-folds 1          # fold0 冒烟
  python chain_t_all.py --skip-t1-128          # 仅 T1
"""
...
def main() -> None:
    assert_t_arm_flags()
    p = argparse.ArgumentParser(description="T-series chain (v3 E_pos token predictor)")
    p.add_argument("--from", dest="from_arm", default="", help="断点续跑，如 T1_128")
