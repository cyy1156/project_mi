...
# S4_focal: FocalLoss 定义在 shallow_variants.py
try:
    from shallow_variants import FocalLoss
except Exception:
    FocalLoss = None

# 结果登记表
from registry import count_params, update_registry
...
def append_md(md_path: Path, text: str, out_root: Path, log_path: Path) -> None:
    ...
    latest.write_text(
        ...
        f"**正式表**：[`09_旁路_shallow_网络结构增强_Three_openbmi_accpaper/总结/结果登记表.md`](./09_旁路_shallow_网络结构增强_Three_openbmi_accpaper/总结/结果登记表.md)（09 旁路 · Shallow 结构增强）\n\n"
...
def collect_preds(
    ...
    non_blocking: bool = True, hier_head: bool = False, n_classes: int = 2,
):
    model.eval()
    ys, ps = [], []
    for x, y in loader:
        out = model(x.to(device, non_blocking=non_blocking))
        if hier_head and isinstance(out, tuple) and n_classes == 3:
            # S3_hier Three: 分层推理
            task_logits, lr_logits = out
            task_pred = task_logits.argmax(dim=1)  # 0=idle, 1=MI
            three_pred = torch.zeros_like(task_pred)
            mi_mask = task_pred == 1
            if mi_mask.any():
                three_pred[mi_mask] = lr_logits[mi_mask].argmax(dim=1) + 1
            ps.append(three_pred.cpu().numpy())
        elif isinstance(out, tuple):
            # S3_hier Task: 只用 task_logits
            logits = out[0]
            if logits.ndim > 2:
...
@torch.no_grad()
def collect_preds_with_probs(
    model: nn.Module, loader: DataLoader, device: torch.device, *, non_blocking: bool = True
):
    """同 collect_preds，但额外返回 softmax 概率（S4_conf_agg 用）。"""
    model.eval()
    ys, ps, probs = [], [], []
    for x, y in loader:
        logits = model(x.to(device, non_blocking=non_blocking))
        if isinstance(logits, tuple):
            # S3_hier: 不支持 conf_agg，回退到 task_logits
            logits = logits[0]
        if logits.ndim > 2:
            logits = logits.reshape(logits.shape[0], -1)
...
def _conf_weighted_trial_pred(
    probs: np.ndarray, subjects: np.ndarray, trial_ids: np.ndarray
) -> tuple[np.ndarray, list]:
    """S4_conf_agg: 对每个 trial 累加 softmax 概率，取 argmax 作为试次预测。"""
    ...
    for subj, tid in order:
        ...
        conf_sum = probs[mask].sum(axis=0)
        trial_pred.append(int(conf_sum.argmax()))
    return np.asarray(trial_pred, dtype=np.int64), order
    ...
    *,
    ...
    def _compute_loss(x, y):
        ...
        if hier_head and isinstance(out, tuple) and n_classes == 3:
            # S3_hier Three: 多任务损失
            task_logits, lr_logits = out
            task_label = (y > 0).long()  # 0=idle, 1=MI
            mi_mask = task_label == 1
            loss = criterion(task_logits, task_label)
            if mi_mask.any():
                lr_label = (y[mi_mask] - 1).long()  # 0=left, 1=right
                loss = loss + criterion(lr_logits[mi_mask], lr_label)
            return loss
        if isinstance(out, tuple):
            out = out[0]  # S3_hier Task: 只用 task_logits
        if out.ndim > 2:
...
def _eval_split(
    ...
    *,
    ...
    if s4_mode == "s4_conf_agg":
        # S4_conf_agg: 置信度加权试次聚合
        yt, yp, probs = collect_preds_with_probs(
            model, loader, device, non_blocking=hp.non_blocking
        )
        assert len(yt) == len(subs) == len(tids)
        trial_pred, order = _conf_weighted_trial_pred(probs, subs, tids)
        # 将试次级预测回填到窗级（使 aggregate_windows_to_trials 的多数投票与之等价）
        yp_for_trial = yp.copy()
        subs_str = np.asarray([str(s) for s in subs])
        tids_arr = np.asarray(tids).astype(np.int64).reshape(-1)
        for i, (subj, tid) in enumerate(order):
            m = (subs_str == subj) & (tids_arr == tid)
            yp_for_trial[m] = trial_pred[i]
        trial = aggregate_windows_to_trials(
            yt, yp_for_trial, subs, tids, n_classes=n_classes
        )
        # 窗级指标用原始 argmax（不改）
        if n_classes == 2:
...
def train_one_fold(
    ...
    *,
    ...
    try:
        ...
        # S4: 根据模式选择 loss（方案 §S4）
        if s4_mode == "s4_focal" and FocalLoss is not None:
            criterion = FocalLoss(gamma=s4_focal_gamma)
        elif s4_mode == "s4_class_weight":
            classes, counts = np.unique(y_tr, return_counts=True)
            w = len(y_tr) / (len(classes) * counts)
            criterion = nn.CrossEntropyLoss(
                weight=torch.tensor(w, dtype=torch.float32).to(device)
            )
        elif s4_mode == "s4_softvote_loss":
            # 方案注明「需 custom batch/sampler 或 trial 包装」；简化=标准CE
            print("  [WARN] s4_softvote_loss: 简化实现=标准CE（完整需 trial-level 分组）")
            criterion = nn.CrossEntropyLoss()
        else:
            criterion = nn.CrossEntropyLoss()
...
def run_baseline_main(
    *,
    ...
    p = argparse.ArgumentParser(
        description=f"{model_name} Shallow 结构增强 · Three 专攻（OpenBMI；balbatch；Acc_paper）"
    )
    p.add_argument(
        "--data",
    ...
    # 参数量统计（用于登记表）
    n_times_for_count = int(X.shape[-1])
    try:
        _tmp_model = build_model(8, n_times_for_count, 3, hp.drop_prob)
        n_params = count_params(_tmp_model)
        del _tmp_model
        print(f"[params] n_params={n_params}", flush=True)
    except Exception as e:
        n_params = 0
        print(f"[params] 参数量统计失败: {e}", flush=True)

    stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    out_name = f"{model_name}_net_enhance_three"
    out_root = (
        TRAIN_LAB
        / "out"
        / OUT_ROOT_TAG
    ...
    md_path = (
        records_root
        / "runs"
        / "5060_shallow_net_enhance_three"
        / f"{stamp}_{out_name}"
        ...
        for info in _iter_folds():
            folds.append(
                train_one_fold(
                    ...
                    s4_mode=kwargs.get("s4_mode"),
                    s4_focal_gamma=kwargs.get("s4_focal_gamma", 1.0),
                    hier_head=kwargs.get("hier_head", False),
                )
            )
    ...
    # S4/S3_hier: 从 extra_meta 和 model_name 提取模式
    s4_mode = extra_meta.get("s4_mode") if extra_meta else None
    s4_focal_gamma = extra_meta.get("s4_focal_gamma", 1.0) if extra_meta else 1.0
    hier_head = "s3_hier" in model_name
    ...
    meta = {
        "model_name": out_name,
        "arm_name": arm_name or model_name,
        "data_tag": data_tag,
        "stamp": stamp,
    ...
    # 结果登记表写入（方案 section3 + section6）
    if arm_name:
        out_root_tag_dir = TRAIN_LAB / "out" / OUT_ROOT_TAG
        records_root = REPO_ROOT / "资料" / "模型训练"
        registry_path = (
            records_root
            / "09_旁路_shallow_网络结构增强_Three_openbmi_accpaper"
            / "总结"
            / "结果登记表.md"
        )
        is_s0 = arm_name.lower().strip() == "s0"
        try:
            update_registry(
                arm_name=arm_name,
                task_summary=sum_task,
                three_summary=sum_three,
                n_params=n_params,
                out_root_tag_dir=out_root_tag_dir,
                registry_path=registry_path,
                is_s0=is_s0,
            )
        except Exception as e:
            print(f"[registry] 登记表写入失败: {e}", flush=True)
            log_line(log_path, f"registry write failed: {e}")
