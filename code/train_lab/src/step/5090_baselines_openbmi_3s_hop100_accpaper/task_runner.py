...
from shared_hparams import SHARED, SharedTrainHP, OUT_ROOT_TAG, SCHEME24_RUNS_TAG
from md_fold_detail import task_fold_md_lines, three_fold_md_lines
from trial_metrics import aggregate_windows_to_trials
from t0_sec import compute_window_t0_sec, t0_train_weight
...
class PackedArrayDataset(Dataset):
    ...
    def __init__(
        ...
        *,
        ...
        if self.sample_weights is not None:
            assert len(self.sample_weights) == n
    ...
    def __getitem__(self, i: int):
        ...
        return (
            torch.from_numpy(x),
            y,
            torch.tensor(float(self.sample_weights[i]), dtype=torch.float32),
        )
...
def append_md(md_path: Path, text: str, out_root: Path, log_path: Path) -> None:
    ...
    latest.write_text(
        ...
        f"**正式表**：[`5070_本机配置与实验计划.md`](./5070_本机配置与实验计划.md)（5070 · Fast）\n\n"
        f"- 本机（5070）本次记录：[`{rel}`](./{rel})\n"
        f"  - 权重：`{out_root}`\n"
        f"  - 日志：`{log_path}`\n",
        encoding="utf-8",
    )
...
def resume_fold_from_ckpt(
    ...
    *,
    ...
    """断点续训：已有 best ckpt 时跳过训练，仅复评 test。"""
    fold = int(fold_info["fold"])
    fold_dir = out_dir / f"fold{fold}"
    ckpt_path = fold_dir / ckpt_name
    if not ckpt_path.is_file():
        raise FileNotFoundError(f"resume fold{fold} missing {ckpt_path}")
    ckpt = torch.load(ckpt_path, map_location="cpu", weights_only=False)
    if X is None and not x_path:
        raise RuntimeError("resume_fold_from_ckpt needs X or x_path")
    n_times = int(np.load(x_path, mmap_mode="r").shape[-1]) if x_path else int(X.shape[-1])
    ...
    val_m = ckpt.get("val_trial_metrics") or {}
    print(
        f"[resume fold{fold}] skip train · val_AccPaper={float(val_m.get('acc_paper', 0)):.4f} "
        f"test_AccPaper={float(te_trial['acc_paper']):.4f}",
        flush=True,
    )
    return {
        "fold": fold,
        "best_val_acc_paper": float(val_m.get("acc_paper", 0.0)),
        "best_val_balacc_maj": float(val_m.get("balanced_accuracy", 0.0)),
        "best_val_loss": float(ckpt.get("best_val_loss", 0.0)) if isinstance(ckpt, dict) else 0.0,
        "best_epoch": int(ckpt.get("epoch", 0)) if isinstance(ckpt, dict) else 0,
        "stopped_epoch": int(ckpt.get("epoch", 0)) if isinstance(ckpt, dict) else 0,
        "best_val_trial_metrics": val_m,
...
def train_one_fold(
    ...
    *,
    ...
    if float(getattr(hp, "t0_filter_max", 0.0) or 0.0) > 0.0:
        ...
        if len(tr_idx) == 0:
            raise RuntimeError(f"fold{fold}: t0_filter_max={t0_cap} 清空 train")
    y_tr = y[tr_idx]
    w_tr = None
    if float(getattr(hp, "t0_weight_alpha", 0.0) or 0.0) > 0.0:
        w_tr = t0_train_weight(t0_all[tr_idx], float(hp.t0_weight_alpha))
    ...
    try:
        ...
        if input_kind == "feat":
            ...
            train_ds = PackedArrayDataset(
                y_tr, x_path=pack_tr_path, sample_weights=w_tr
            )
            X = None  # noqa: F841
            if src_box is not None:
                src_box.clear()
...
def run_baseline_main(
    *,
    ...
    p = argparse.ArgumentParser(
        description=f"{model_name} 3s/hop100 Acc_paper 重训（OpenBMI；方案24·5090；balbatch）"
    )
    p.add_argument(
        "--data",
        default=SHARED.data_tag,
        choices=("openbmi_3s_hop100",),
        help="仅 OpenBMI 3s/hop100；禁用 BCI2a/Stieger 混训",
    )
    ...
    p.add_argument(
        ...
        type=float,
        default=-1.0,
        help="T 臂：损失软降权 α（>0 启用；默认 0 即均匀）",
    )
    p.add_argument(
        "--t0-filter-max",
        type=float,
        default=-1.0,
        help="硬滤 train 窗 t0_sec≤阈值（如 0.7；默认不过滤）",
    )
    p.add_argument(
        "--dump-probs",
        action="store_true",
        help="仅重放 --replay-run-dir 下 ckpt 导出 prob dump（不训练）",
    )
    p.add_argument(
        "--replay-run-dir",
        type=Path,
        default=None,
        help="dump-probs 目标 run 的 stage 目录（如 .../three）",
    )
    p.add_argument(
        "--replay-stage",
        default="three",
    ...
    p.add_argument(
        ...
        type=Path,
        default=None,
        help="断点续训：指向已有 run_*/three，跳过已有 best_three.pt 的折",
    )
    args = p.parse_args()

    if args.dump_probs:
        if args.replay_run_dir is None:
            raise SystemExit("--dump-probs 需要 --replay-run-dir")
    ...
    if args.t0_weight >= 0.0:
        repl["t0_weight_alpha"] = float(args.t0_weight)
    if args.t0_filter_max >= 0.0:
        repl["t0_filter_max"] = float(args.t0_filter_max)
    if repl:
        hp = replace(hp, **repl)
    ...
    if n_subjects < 3:
        raise RuntimeError(
            f"openbmi_3s_hop100 当前仅 {n_subjects} 名被试（{data_dir}）。"
            "被试独立五折至少需要多名被试；请先跑："
    ...
    out_name = f"{model_name}_openbmi_3s_hop100_balbatch_accpaper"
    resume_three_dir: Path | None = None
    if args.resume_three_dir is not None:
        resume_three_dir = Path(args.resume_three_dir).resolve()
        if not resume_three_dir.is_dir():
            raise SystemExit(f"--resume-three-dir not found: {resume_three_dir}")
        out_root = resume_three_dir.parent
        stamp = out_root.name.replace("run_", "", 1)
        print(f"[resume] three_dir={resume_three_dir} out_root={out_root}", flush=True)
    else:
        out_root = (
            TRAIN_LAB
            / "out"
    ...
    def run_kfold_limited(**kwargs):
        ...
        for info in _iter_folds():
            fold = int(info["fold"])
            ckpt_path = out_dir / f"fold{fold}" / ckpt_name
            if resume_three_dir is not None and ckpt_path.is_file():
                folds.append(
                    resume_fold_from_ckpt(
                        info,
                        kwargs["X"],
                        kwargs["y"],
                        kwargs["subjects"],
    ...
    append_md(
        ...
        "\n".join(
            [
                ...
                f"- 训练设备：**NVIDIA RTX 5090**（正式结果以本机 Fast 为准）",
                f"- data：`{data_dir}`（**仅 OpenBMI / 3s hop100**；blocks=EEG_MI_train）",
                f"- protocol：`{hp.protocol}` | early_stop=**Acc_paper** | **balbatch** | no_rap",
                f"- 读数口径：`Tw=3s hop=100ms openbmi_sess01+02 subject_key=openbmi:subjNN early_stop=val_acc_paper select=test_acc_paper balbatch patience={hp.patience}`",
