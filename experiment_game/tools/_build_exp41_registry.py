...
def main() -> None:
    ...
    lines += [
        ...
        "## 0. 一句话",
        "",
        f"**15 人真人队列已统一结构与末档读数；末档 FT F5 MI 均值 {mean_mi:.1%}（{n_pass}/15 末档 PASS）。** "
    ...
    lines += [
        ...
        "| 仍排除 `record_excluded` / 无 eeg | 如 zyj0902 w03 |",
        "| 命名 `ws*` + 跳过首场 v4 | syj0828 · fnz0828 |",
        "| 合并场 `w02+w03` | ytl0901 |",
        "| 跳半场 / excluded | ycx0831 跳 w06；zyj0902 跳 w03 |",
        "| 六档至 w07 | lsm0903 |",
        "",
        "## 4. 与历史实验对齐",
        "",
        ...
        "## 5. 结论",
        "",
        "1. **结构已统一**：15 人 session 键、爬坡特例、强制纳入名单、落盘 JSON 均有单一入口。  ",
        "2. **表现分化大**：末档 MI 从 8.6%–91.7%；约六成末档过门控，但不等于都可 promote。  ",
        "3. **重跑缺口**：cyy0830 / npl0831 当前可重跑档=0，cjf/wzr 短于历史落盘——若需刷新，须先修 index / 纳入策略。  ",
