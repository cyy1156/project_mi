...
**实验板块** = 真机采集 + 真 LSL；**仿真板块** = 选定 BCI2a 被试的一个 **run** 当作一场 **session**（trial 数 **6–48 可选**），按 OpenBMI-Align v3 在线回放判定、落盘、采后 FT；**省略** session 30s 基线与块间 90s，静息校准用 Cue 前 Rest；同一场仿真实验 **只服务一个被试**，**run 可选序但不可重复**，权重链与真机一致。
...
| **块结构 / 时序 / 计分** | OpenBMI-Align v1 | **单 trial 时序相同**；**跳过** 开场 30s 基线、块间 90s（见 §3.3、§4.5） |
...
| **3 块间 90s** | 真休息 · `block_gap` | **跳过**（不播 ITI+Rest 拼接、不黑屏计时） | 块 1 结束 **直接进入** 块 2 |
| **4 块 2 · N₂ trial** | 另一条件 | **run3 接下来 N₂ 个 trial** | 默认 N₂=18；N₁+N₂ ≤ 48 |
| **5 报告** | v3_report | 同报告模板 + **`sim_source_run=3`** | |

> **仿真与真机两处刻意省略：** 仅省略 **session 级** 30s 基线与 **块间** 90s；**单 trial 内** Cue 前 Rest 4s → prep → Cue/MI → ITI 仍完整回放（`schedule_align`）。

**单场时序（单 trial · 与 Align 一致）：**

```text
Cue前 Rest 4s → prep 2s → Cue=MI → MI 4s → ITI 3s  （合计 13s）
```

**在线：** 3 s/hop100，判 3.0…4.0 s，**多数票 0/1**；操作台 **本场得分** 累计。
...
| **块间 / session 级** | 仍遵守 §4.5（仿真跳过 30s baseline、90s gap） | 同左 |
...
| **块间 `block_gap_s` 90s** | **跳过** · 状态机 **块 1 末 trial 结束 → 立即块 2 首 trial** | 无替代；操作台块间进度条 **隐藏或显示「仿真跳过」** |
| **单 trial 内 Rest 4s** | **保留** · 正常回放 cue 前波形 | 与真机 Align 一致，供在线窗与 FT |

```text
真机：  [30s baseline] → Block1(18t) → [90s gap] → Block2(18t)
...
| **仿真专用** | 跳过 session 30s 基线与块间 90s；静息校准仅依赖 trial 内 Cue 前 Rest |
...
| Q2 | 块间 90s（真机）/ 仿真块间？ | 真机仍 **黑屏计时**；**仿真整段跳过**（§4.5） |
...
| 2026-08-27 | 修订：仿真跳过 session 30s/块间 90s；trial 可选 6–48；冻结 Q1–Q7 |
