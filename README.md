...
| ❌ **没有** | `experiment_game/data/`（原始采集数据 + 派生训练样本，约 332MB）与训练输出/权重/prob dump（约 490MB） |

后两类是**本地专属数据与可复现的中间产物**，体量约占仓库 90%，故不入库以保持仓库轻量。它们仍完整保留在本地，获取/重建方式见下方「数据与产物」一节。

![系统与实验总览](资料/初赛材料/01_技术报告/figures/fig00_system_experiment_overview.png)
...
| 在线系统架构 | 实验操作台界面 |
|---|---|
| ![在线系统架构](资料/初赛材料/01_技术报告/figures/fig01_system_architecture.png) | ![实验操作台界面](资料/初赛材料/01_技术报告/figures/fig_ui_screenshot.png) |

## 主要结果

| 主实验结果（混淆矩阵） | 少样本自适应曲线 |
|---|---|
| ![主结果混淆矩阵](资料/初赛材料/01_技术报告/figures/fig06_main_results_cm.png) | ![少样本自适应曲线](资料/初赛材料/01_技术报告/figures/fig09_leave_next_curves.png) |

> 完整指标口径与实验设置见 `资料/初赛材料/01_技术报告/` 中的技术报告。
