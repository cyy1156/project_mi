# train_shallow_hop100_accpaper.py

> 2s/hop100 · Acc_paper 五折（standalone Shallow）。对齐 aselines_2s_hop100_accpaper/task_runner.py：先 Task 二分类，再 Three 三分类，并写 MD 实验记录。

- 数据：ci2a_2s_hop100；Train 窗 CE + balbatch；Val 早停 Acc_paper
- 输出：self_model/out/.../run_STAMP/（	ask/、	hree/、meta.json、五折 MD）
- 同步：资料/模型训练/runs/... 与 五折实验记录_最新.md（若可写）
...
完整可运行代码见同目录 	rain_shallow_hop100_accpaper.py（以下为同步副本）。
...
def main() -> None:
    ...
    out_name = "standalone_shallow_2s_hop100_balbatch_accpaper"
    ...
    header = "\n".join(
        [
            ...
            f"- device：`{device}`",
            f"- data：`{data_dir}`（仅 BCI2a T / hop100）",
            f"- protocol：`{hp.protocol}` | early_stop=**Acc_paper** | **balbatch**",
            f"- model：`standalone ShallowFBCSPNet` | no braindecode",
            f"- Train：窗 CE + batch balance；Val/Test：trial Acc_paper",
            f"- 流程：先 Task（二分类）再 Three（三分类）"
    ...
    log_line(log_path, f"start model={out_name} device={device} data={data_dir}")
    ...
    md_tail = [
        ...
        f"- Test 窗级 BalAcc：`{sum_task['test_window_balacc_mean']:.4f} ± {sum_task['test_window_balacc_std']:.4f}`",
        f"- Test 窗级 Sens/Spec/F1：`{_f(sum_task.get('test_window_sensitivity_mean'))}` / "
    ...
    if sum_three is not None:
        md_tail.extend(
            [
                ...
                f"- Test 窗级 BalAcc：`{sum_three['test_window_balacc_mean']:.4f} ± {sum_three['test_window_balacc_std']:.4f}`",
                f"- Test F1-macro(maj)：`{_f(sum_three.get('test_f1_macro_maj_mean'))}`",
                f"- Test Spec(trial/win)：`{_f(sum_three.get('test_trial_specificity_mean'))}` / "
