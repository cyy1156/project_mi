"""OpenBMI 固定 2s 窗（Cue+2–4s / Rest 前 2s）批处理。
...
用法（在 preprocess_lab 根目录）：
  ...
  python -m src.datasets.openbmi.batch_2s_fixed_cue2to4 --merge-only
"""
...
def load_manifest(path: Path, *, protocol: str = "openbmi_2s_fixed_cue2to4_noz") -> dict:
    if not path.exists():
...
def merge_shards(
    ...
    *,
    val_ratio: float = 0.2,
    seed: int = 42,
    protocol: str = "openbmi_2s_fixed_cue2to4_noz",
    zscore: bool = False,
) -> None:
    ...
    sanity_check_outputs(np.asarray(X[: min(8, len(X))]), y_task[: min(8, len(y_task))], y_three[: min(8, len(y_three))])
    # 全量 sanity（不整表拷贝 X）
...
def run_batch(
    ...
    *,
    ...
    protocol = (
        "openbmi_2s_fixed_cue2to4" if zscore else "openbmi_2s_fixed_cue2to4_noz"
    )
    ...
    for fpath in files:
        ...
        try:
            X, yt, y3, sid, tid, stats = preprocess_file_fixed_cue2to4(
                fpath, zscore=zscore
            )
...
def main() -> None:
    p = argparse.ArgumentParser(description="OpenBMI 固定 2s 窗（Cue+2–4s）预处理")
    p.add_argument(
        "--glob",
        default=str(
    ...
    p.add_argument(
        "--out",
        default=str(_PREPROCESS_ROOT / "out" / "openbmi_2s_fixed_cue2to4_noz"),
    )
    ...
    p.add_argument(
        "--with-zscore",
        action="store_true",
        help="开启窗内 z-score（默认关闭，对齐方案 08）",
    )
    p.add_argument(
        "--subjects",
        default="all",
