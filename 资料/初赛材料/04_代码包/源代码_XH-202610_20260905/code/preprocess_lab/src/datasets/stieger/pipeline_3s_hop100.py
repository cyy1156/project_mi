"""Stieger 3 s / 100 ms（旁路） 协议（与旧「反馈末 2s/4s」流水线分离）。

Rest：Cue(=fb0) 前 4 s → slide_3s_hop100；不再使用 down 作 Rest。
Task：左/右反馈全程 [fb0, fb1) → slide_3s_hop100；丢 up/down、artifact、反馈 < 3 s。

同时写出窗内 z-score 的 X（推理）与无 z-score 的 X_noz（在线门控）。
trial_id：Task=tid_base，Rest=tid_base+1（Acc_paper 分段）；门控用 cue_id=tid//2*2。
"""
...
def _process_one_trial_3s_hop100(
    tr: StiegerTrial,
    *,
    ...
    # Rest: Cue 前 4 s；trial_id = tid_base+1（与 Task 成对）
...
def preprocess_session_3s_hop100(
    mat_path: Path | str,
    *,
    feedback_t_ms: float = 2000.0,
    baseline_sec: float = 0.5,
    ...
    stats = {
        ...
        "n_task_wins": 0,
        "n_rest_wins": 0,
        "protocol": "slide_3s_hop100",
        "has_x_noz": True,
    }
    tid = 0
    for tr in trials:
        out = _process_one_trial_3s_hop100(
            tr,
