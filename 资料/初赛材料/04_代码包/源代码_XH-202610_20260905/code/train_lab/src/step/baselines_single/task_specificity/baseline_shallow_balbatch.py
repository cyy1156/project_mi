"""ShallowFBCSPNet：Task-only 五折，batch balance 对照臂（不改 baseline_shallow.py）。

臂定义：
  B1: 普通 CE + train 1:1 batch balance + BalAcc 早停
  B2: 加权 CE (w0=2,w1=1) + train 1:1 batch balance + BalAcc 早停
...
ARMS: dict[str, ArmConfig] = {
    "B1": ArmConfig(
        name="B1",
        model_name="shallow_balbatch_balacc",
        use_weighted_ce=False,
        task_w0=1.0,
        task_w1=1.0,
        description="普通CE + train batch balance 1:1 + BalAcc早停",
    ),
    "B2": ArmConfig(
        name="B2",
        model_name="shallow_wce2_balbatch_balacc",
        use_weighted_ce=True,
        task_w0=1.05,
        task_w1=1.0,
        description="加权CE w0=2,w1=1 + train batch balance 1:1 + BalAcc早停",
    ),
}
...
def train_task_one_fold(
    ...
    y_tr = y[masks["train"]]
    train_ds = ArrayTaskDataset(X[masks["train"]], y_tr)
    ...
    append_md(
        ...
        "\n".join(
            [
                ...
                f"- model：`{arm.model_name}`（新脚本 baseline_shallow_balbatch.py；不改 baseline_shallow.py）",
                f"- 臂：`{arm.name}` — {arm.description}",
                f"- Task 采样：train WeightedRandomSampler（类逆频，约 1:1）；val/test 真实比例",
...
def main() -> None:
    p = argparse.ArgumentParser(
        description="Shallow batch-balance 对照：仅 Task；臂 B1/B2（不改 baseline_shallow.py）"
    )
    ...
    p.add_argument(
        ...
        default="B2",
        choices=("B1", "B2", "both"),
        help="B1=普通CE+balance；B2=w0=2加权CE+balance；both=依次跑两臂",
    )
    args = p.parse_args()
    ...
    arm_names = ("B1", "B2") if args.arm == "both" else (args.arm,)
