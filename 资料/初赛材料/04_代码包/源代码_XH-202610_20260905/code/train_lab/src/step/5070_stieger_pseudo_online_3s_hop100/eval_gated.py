"""S08-03 / S08-06：在线门控 H0–H3。

--mode zeroshot → S08-03（S3 权重 · 全量）
--mode ft       → S08-06（只读半程 FT ckpt · 后半）
"""
...
def main() -> None:
    p = argparse.ArgumentParser(description="S08 在线门控 H0–H3（5070）")
    ...
    p.add_argument(
        "--ft-stamp",
        default="",
        help="S08-02 FT 权重 stamp（目录名）；默认取最新",
    )
    ...
    if args.mode == "zeroshot":
        arm = "S08-03"
        results_dir = RESULTS_ROOT / "S08-03_gate"
        run_dir = resolve_openbmi_s3_run("shallow", run_stamp=args.run_stamp or None)
        ft_root = None
    else:
        arm = "S08-06"
        results_dir = RESULTS_ROOT / "S08-06_ft_gated"
        run_dir = None
