"""参考TepmoralEncoder_GCBNet.py+baseline_dgcnn.py"""
...
MODEL_NAME = "dgcnn_raw"
NODE_DIM = 64  # TemporalEncoder 输出 D = DGCNN.in_channels
...
# ---------------------------------------------------------------------------
# 3) Dataset / 锁种 / 图网络（DGCNN in_channels=D）
# ---------------------------------------------------------------------------
...
class DGCNNRaw(nn.Module):
    """(B,8,500) → TemporalEncoder → DGCNN → logits"""
...
def run_task_kfold(X, y, subjects, device, hp: SharedTrainHP, out_dir: Path, data_tag: str) -> dict:
    ...
    summary = {
        ...
        "hparams": shared_as_dict(),
        "dgcnn_raw": {
            "backbone": "TemporalEncoder+DGCNN",
            "node_dim": NODE_DIM,
            "k": 2,
            "layers": [128],
        },
...
def main() -> None:
    p = argparse.ArgumentParser(description="DGCNNRaw：原始时域 + TemporalEncoder + Task/Three 独立五折")
    ...
    append_md(
        ...
        "\n".join(
            [
                ...
                f"- model：`{MODEL_NAME}`（TemporalEncoder + DGCNN）",
                f"- 输入：raw `{X.shape}` → Encoder → 节点特征 D={NODE_DIM}（非 bandpower）",
                f"- 结构：DGCNNRaw(k=2, layers=[128], in_channels={NODE_DIM}, relu_is=1)",
