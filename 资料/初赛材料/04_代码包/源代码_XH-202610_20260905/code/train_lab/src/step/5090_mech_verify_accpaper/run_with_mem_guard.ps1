...
function Kill-Tree([int]$RootPid) {
  ...
  Get-CimInstance Win32_Process -Filter "Name='python.exe'" -EA SilentlyContinue |
    Where-Object {
      $_.CommandLine -and (
        $_.CommandLine -match '5090_mech_verify_accpaper' -or
...
$existing = @(Get-CimInstance Win32_Process -Filter "Name='python.exe'" -EA SilentlyContinue |
  Where-Object {
    $_.CommandLine -and (
      $_.CommandLine -match '5090_mech_verify_accpaper\\run_arm\.py' -or
      $_.CommandLine -match '5090_mask_future_dual_expert_accpaper\\run_arm\.py'
    )
  })
...
$runnerBody = @"
`$ErrorActionPreference = 'Continue'
try { `$Host.UI.RawUI.WindowTitle = 'scheme23 · $Arm · mem-guarded' } catch {}
...
`$pyArgs = @($argLit)
Write-Host ('[guarded23] start ' + (Get-Date -Format o) + ' arm=$Arm') -ForegroundColor Cyan
Write-Host ('[guarded23] ' + '$py' + ' ' + (`$pyArgs -join ' ')) -ForegroundColor DarkGray
...
Write-Host ('[guarded23] exit=' + `$code + ' ' + (Get-Date -Format o)) -ForegroundColor Yellow
exit `$code
"@
...
while (-not $proc.HasExited) {
  ...
  $pys = @(Get-CimInstance Win32_Process -Filter "Name='python.exe'" -EA SilentlyContinue |
    Where-Object {
      ...
      ($_.CommandLine -and $_.CommandLine -match 'run_arm\.py|5090_mech_verify|5090_mask_future')
    })
