"""本包 baseline_*.py 注册表（结构与 01 同形；训练走本包 task_runner）。"""
...
MODEL_SPEC: dict[str, tuple[str, str | None]] = {
    ...
    # 与 01 一致：*_raw 用 feat Dataset + squeeze（见 baseline_*_raw.py）
...
def _load_local_baseline(name: str):
    ...
    spec = importlib.util.spec_from_file_location(f"accpaper_baseline_{name}", path)
...
def get_structure_note(name: str) -> str:
    if name == "deep":
        return "Deep4Net-compat pool_time_length/stride=1（与 hop100 01 一致）"
