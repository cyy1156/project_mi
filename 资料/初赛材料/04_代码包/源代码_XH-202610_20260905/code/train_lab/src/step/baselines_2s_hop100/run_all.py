"""一次性串跑 2s/hop100 基线（单库；不做 merged）。

用法（在 baselines_2s_hop100/ 目录下）：
  python run_all.py --data bci2a_2s_hop100
  python run_all.py --data stieger_2s_hop100
  python run_all.py --data both
  python run_all.py --data bci2a_2s_hop100 --models eegnet,dbn_raw,gcbnet_raw
"""
...
DATA_CHOICES = ("bci2a_2s_hop100", "stieger_2s_hop100", "both")
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="串跑 baselines_2s_hop100（BalAcc+balbatch；含 *_raw）")
    p.add_argument("--data", default="bci2a_2s_hop100", choices=DATA_CHOICES)
    p.add_argument(
        "--models",
...
def main() -> None:
    ...
    data_tags = (
        ("bci2a_2s_hop100", "stieger_2s_hop100") if args.data == "both" else (args.data,)
    )
    failed: list[str] = []
