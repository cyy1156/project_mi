"""Three 下游：J0 同骨干监督 / J2 冻住探测 / J3 解冻微调。
...
from jepa_model import JepaClassifier, freeze_backbone, unfreeze_backbone
...
def train_one_fold(
    *,
    ...
    if arm == "j0":
        ...
        model = JepaClassifier(backbone, n_outputs=3, drop_prob=hp.drop_prob).to(device)
        enc_lr = hp.lr
    elif arm == "j2_random":
        backbone = build_backbone(hp).to(device)
        model = JepaClassifier(backbone, n_outputs=3, drop_prob=hp.drop_prob).to(device)
    ...
    train_loader = make_loader(
        WindowDataset(X, tr_idx, y=y, x_path=x_path),
        hp.batch_train,
        shuffle=False,
        hp=hp,
