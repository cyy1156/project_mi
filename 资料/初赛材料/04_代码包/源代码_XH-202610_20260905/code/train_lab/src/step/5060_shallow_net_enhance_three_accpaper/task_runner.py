"""Shallow 网络结构增强旁路（方案 09）：Task + Three 五折。"""
...
HOP100_ACC = STEP_DIR / "baselines_2s_hop100_accpaper"
OLD_BASELINES = STEP_DIR / "baselines_single"
OFFICIAL_5060 = STEP_DIR / "5060_baselines_openbmi_2s_hop100_accpaper"

for p in (STEP_DIR, PRE_ROOT, HOP100, HOP100_ACC, OLD_BASELINES, OFFICIAL_5060):
    sp = str(p)
...
class PackedArrayDataset(Dataset):
    ...
    def __init__(
        ...
        *,
        ...
        self.w = None if w_pack is None else np.asarray(w_pack, dtype=np.float32)
    ...
    def __getitem__(self, i: int):
        ...
        return torch.from_numpy(x), y, torch.tensor(self.w[i], dtype=torch.float32)
...
def _n_chans_of(X_src: np.ndarray) -> int:
    if X_src.ndim == 4 and X_src.shape[1] == 1:
        return int(X_src.shape[2])
    if X_src.ndim == 3:
        return int(X_src.shape[1])
    raise ValueError(f"unexpected X shape: {X_src.shape}")
...
def materialize_time_pack(
    ...
    *,
    ...
    """将全局下标对应的时域窗顺序写入磁盘 memmap，返回路径。"""
    indices = np.asarray(indices, dtype=np.int64).reshape(-1)
    n = int(len(indices))
    t = int(X_src.shape[-1])
    n_ch = _n_chans_of(X_src)
    out_path.parent.mkdir(parents=True, exist_ok=True)
    if out_path.exists():
        out_path.unlink()
    fp = np.lib.format.open_memmap(
        out_path, mode="w+", dtype=dtype, shape=(n, n_ch, t)
    )
    for s in range(0, n, _GATHER_CHUNK):
        e = min(s + _GATHER_CHUNK, n)
        idxs = indices[s:e]
        block = _squeeze_time_windows(np.array(X_src[idxs]))
        if block.shape[1] != n_ch:
            raise ValueError(f"pack ch mismatch: {block.shape} vs n_ch={n_ch}")
...
def append_md(md_path: Path, text: str, out_root: Path, log_path: Path) -> None:
    ...
    latest.write_text(
        f"# 最新实验入口\n\n"
        f"**旁路**：[`05_旁路_shallow_MI特征工程_openbmi_accpaper/`](./05_旁路_shallow_MI特征工程_openbmi_accpaper/)  \n"
        f"正式表仍见：[`5060_openbmi_accpaper_实验与权重清单.md`](./5060_openbmi_accpaper_实验与权重清单.md)\n\n"
        f"- 本次旁路记录：[`{rel}`](./{rel})\n"
        f"  - 权重：`{out_root}`\n"
        f"  - 日志：`{log_path}`\n",
        encoding="utf-8",
...
def eval_loss_and_preds(
    ...
    *,
    ...
    """一次前向同时得到 val_loss 与预测，避免原先 run_epoch+collect_preds 双遍扫描。"""
    model.eval()
    ...
    with ctx:
        for batch in loader:
            ...
            def _loss(logits):
                ...
                per = nn.functional.cross_entropy(logits, y, reduction="none")
                return (per * w).mean()
...
def _eval_split(
    ...
    *,
    ...
    loss, yt, yp = eval_loss_and_preds(
        model,
        loader,
        device,
    ...
    *,
    ...
    try:
        ...
        if input_kind == "feat":
            ...
            w_tr = None if sample_weights is None else sample_weights[tr_idx]
            train_ds = PackedArrayDataset(y_tr, x_path=pack_tr_path, w_pack=w_tr)
            print(
                f"  pack done train={pack_tr_path.name} val={pack_va_path.name}"
                + (f" weighted={w_tr is not None}" if True else ""),
                flush=True,
            )

        use_w = sample_weights is not None and input_kind != "feat"
        ...
        model = build_model(n_chans, n_times, n_outputs, hp.drop_prob).to(device)
        criterion = nn.CrossEntropyLoss()
    ...
    *,
    ...
    if sample_weights is not None:
        ...
        assert len(sample_weights) == len(X), (
            f"sample_weights len={len(sample_weights)} != N={len(X)}"
        )
