...
> **统计口径**：[`统计口径方案A_20260831.md`](统计口径方案A_20260831.md)

> 日期：2026-08-27 · **修订：2026-08-29** · 状态：**已落地**  
> 总表：[`框架冻结确认_20260829.md`](框架冻结确认_20260829.md)  
> 相关：`fnz实验与微调采集方案_20260827.md` · `ft_subject_from_v3.py` · `operator.html`

---

## 0. 文档目的

定义以被试英文缩写为中心的采集 + **采后**微调工作流。权威数据源 = **`data/subjects/{id}/`**（Excel 仅导出，F13）。
...
| # | 目标 | 说明 |
|---|------|------|
| G1 | **登录即身份** | 输入缩写（如 `syj0828`）→ 创建/加载工作区 |
| G2 | **目录按人隔离** | session / current / ft_runs 互不串人 |
| G3 | **Session 后可选 FT** | 摘要页选 session；**默认排除 v4**；支持 Leave-Next |
| G4 | **下次自动带权重** | 有 `models/current` 则默认指向本被试；另有全局 **E1f 四成员** preset |
| G5 | **采后 FT** | 默认底座 E1f/5090 shallow + T0 replay 0.10；**不做 session 内轮间 FT** |
| G6 | **门控后自动晋升（F8）** | 必存 `ft_runs/`；默认 `auto_promote_after_ft` + `force_promote_on_gate_fail`；FAIL 时落盘 `force_promote_warning.json` 仍晋升 current |
| G7 | **同号多 phase** | 允许 `ws01(v4)` 与 `ws01(v3)` 并存；完整目录名唯一 |
...
| **强制替换** | 默认开启（F8）：门控 FAIL 仍 promote，并落盘 `force_promote_warning.json` |
...
| **gate** | `release_gate.json` → `pass` | PASS / FAIL（默认仍自动晋升 current；FAIL 写 `force_promote_warning.json`，见 F8） |
...
| 固定项 | 值 |
|--------|-----|
| 底座 | 默认 **5090 E1f**（`e1f_four_member.json`）；对照可选 5070 `run_20260822_094942` |
| 训练数据 | **仅**勾选的该被试 session（排除 v4） |
| Replay | T0-36，ratio=0.10（Leave-Next 爬坡后几档可为 0） |
| 划分 | **主路径 Leave-Next**（前序 train / 下一场 heldout）；非 leave 回退按试次 70/30 |
| FT 范围 | 默认 **all4**（F8b）；`so` 仅旁路 |
| 默认含 invalid | **是**（协议 cue 标签） |

### 4.3 发布门控（仅 three · 方案 A：门控=raw · 展示=smooth）

门控**不阻止保存**；**默认自动晋升 current**（冻结 **F8**：`auto_promote_after_ft` + `force_promote_on_gate_fail`）。FAIL 时仍晋升并落盘 `force_promote_warning.json`。

> **方案 A**：操作台 / cohort 主展示 **`three_heldout` = 窗级 smooth**；门控 PASS/FAIL 仍由 **raw** `heldout_acc` / `heldout_acc_raw`（及 max_class_frac 等）决定。二者可能数值接近但不等价。

| # | 检查项 | 阈值 | 数据源 | 说明 |
|---|--------|------|--------|------|
| G1 | three heldout acc | ≥ max(0.40, 先验+0.05) | **raw** 窗级 | 方案 A 门控口径 |
| G2 | three heldout 任一类 pred 占比 | < 60% | raw pred | 防塌缩 |
| G3 | three heldout 三类均有 pred | 是 | raw pred | 与 G2 互补 |
| G4 | train − heldout acc | < 0.35 | **raw** 窗级 | train_gap |

**task 头：** 同配方 FT 后报告 heldout acc 与 Rest/MI pred 分布，UI 标「**参考**」——**不作发布判据**。

**工具现状（2026-08-31）：** 操作台 FT 默认 all4 + Leave-Next；门控后按 F8 自动晋升；口径见方案 A。
...
| `window_acc` | 窗级 **因果平滑** argmax（方案 A · 对齐 `acc_window`） | v3_report / 离线回放 | ✅ |
| `primary_acc` / `acc_argmax` | 试次级多数票（含截断 Rest） | v3 report | ✅ |
...
| 7 | CLI / 操作台：必存 ft_runs + **F8 自动/强制晋升** | ✅ 已实现（门控不 exit；FAIL 写 warning） |
...
| 2 | 第 1 场 FT | **允许 + 强提示** |
| 3 | 门控 FAIL | **默认仍 promote**（F8 + warning）；ft_runs 必留 |
| 4 | v2 轮间 → current | **禁止直连**；仅 Session 后 FT + 自动/强制晋升（F8） |
| 5 | 登录 PIN | **否**（二期） |
| 6 | session_id | **`wsNN`** |
| 7 | 微调默认 | **跳过** |
...
| **P1** | FT 面板：必存 ft_runs + F8 自动晋升 + 门控展示 | 采完微调闭环 |
...
| `archive/d8_phase1/v3零样本探针会话设计_20260824.md`（已归档） | v3 采集中不在线 FT（历史） |
