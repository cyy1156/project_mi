...
## 1. 自采范式（OpenBMI-Align v1，冻结）

| 项 | 设定 |
|---|---|
| 通道 | 8 通道：`FC3, C3, CP3, CZ, CPZ, FC4, C4, CP4`（模型输入通道轴顺序，与《技术报告》附录 B 一致，禁止重排） |
| 试次时序 | Rest 4s → prep 2s → Cue 1s → MI 4s → ITI 3s |
| 特征窗 | 3 s / 750 点，hop 100 ms，逐窗 z-score |
| 类别 | 左手 MI / 右手 MI / 空闲（Rest）三分类 |
| 每被试流程 | 校验帽检 → 多场次采集（含保留场次）→ 采后 Leave-Next 增量微调 → 门控判定 → 部署在用个人模型（`models/current/`） |
...
|---|---|
| 被试编号 | syj0828 |
| 场次结构 | 共 5 个场次，其中末位为保留场次 |
| 数据落盘 | `experiment_game/data/subjects/<被试编号>/sessions/<会话名>/`（eeg.csv、events.jsonl、manifest.json） |
| 对齐检查 | PASS（逐试次事件标记与 v3_session.yaml 时序一致） |
| 零样本三分类窗级 | 41.40% |
| Leave-Next 末档三分类窗级 | 92.40%（逐轮爬坡曲线见《技术报告》图 15） |
| 试次投票 | 92.60%；静息试次判对率 94.40%（试次计分 41.5/45） |
| 门控 | 全部 PASS，在用个人模型（`models/current/`）已晋升 |
