"""Stieger 2 s / 100 ms 滑窗（旁路）批处理：每会话写 shard，结束（或 --merge-only）再合并为训练用 npy。"""
...
from src.datasets.stieger.pipeline_2s_hop100 import preprocess_session_2s_hop100 as preprocess_session_1s, sanity_check_outputs
...
def load_manifest(path: Path) -> dict:
    if not path.exists():
        return {"version": 1, "protocol": "slide_2s_hop100", "files": {}}
...
def run_batch(
    ...
    *,
    ...
    if reset:
        ...
        print("已重置 out/stieger_2s_hop100")
    ...
    print(f"[stieger_2s_hop100] 候选 {len(files)} 个文件 → {out_dir}")
...
def main() -> None:
    ...
    p.add_argument("--out", default=str(_PREPROCESS_ROOT / "out" / "stieger_2s_hop100"))
