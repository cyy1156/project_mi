"""S08-01：S3 shallow × Stieger 全量零样本（无门控 ≡ H0 / Q0）。"""
...
RESULTS = RESULTS_ROOT / "S08-01_zeroshot"
...
def main() -> None:
    p = argparse.ArgumentParser(description="S08-01 Stieger 3s 零样本")
    ...
    md = [
        f"# S08-01 Stieger 零样本 · shallow 3s",
        "",
        f"- 时间：`{stamp}`",
        f"- 权重：`{run_dir}`",
        f"- 门控：无（≡ H0 / Q0）",
