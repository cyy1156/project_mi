"""J1 · 纯 JEPA 预训（按折排除 test 被试）。

用法:
  cd code/train_lab/src/step/5060_jepa_three_probe_openbmi_accpaper
  python run_j1_pretrain.py --max-folds 1 --pretrain-epochs 5
  python run_j1_pretrain.py
"""
...
def main():
    p = argparse.ArgumentParser(description="J1 JEPA pretrain (fold-safe)")
    ...
    p.add_argument(
        ...
        type=int,
        default=0,
        help=">0 时每折最多用 N 个窗做冒烟（随机子采样，非正式）",
    )
    args = p.parse_args()
    ...
    x_path = data["x_path"]
    print(f"[j1] X{tuple(X.shape)} loaded", flush=True)
    ...
    for i, info in enumerate(
        ...
        ds = WindowDataset(X, tr_idx, y=None, x_path=x_path)
        ...
        print(f"[j1] model on {device}, steps/epoch≈{max(1, len(ds)//hp.pretrain_batch)}", flush=True)
        ...
        for ep in range(1, hp.pretrain_epochs + 1):
            ...
            for bi, xb in enumerate(loader, 1):
                ...
                if hp.use_amp and device.type == "cuda":
                    with torch.amp.autocast("cuda"):
                        loss, last_info = model.forward_jepa(xb)
                    loss.backward()
                else:
                    loss, last_info = model.forward_jepa(xb)
                    loss.backward()
            ...
            print(
                f"fold{fold} ep {ep:03d}  L_jepa={total/max(n,1):.4f}  "
                f"mask={last_info.get('mask_frac',0):.2f}  "
                f"pred_std={last_info.get('pred_std',0):.3f}  "
                f"tgt_std={last_info.get('tgt_std',0):.3f}",
                flush=True,
            )
        ...
        path = j1_dir / f"fold{fold}_jepa.pt"
