"""解析 OpenBMI S3 Acc_paper 权重（5070 out）。"""
...
def resolve_openbmi_s3_run(model: str = "shallow", *, run_stamp: str | None = None) -> Path:
    ...
    if not base.is_dir():
        raise FileNotFoundError(
            f"缺少 OpenBMI S3（5070）权重目录: {base}\n"
            "请先跑: code/train_lab/src/step/5070_baselines_openbmi_3s_hop100_accpaper/"
            "baseline_shallow.py"
        )
    stamp = run_stamp or (OPENBMI_S3_SHALLOW_RUN or None)
    if stamp:
        run_dir = base / stamp if stamp.startswith("run_") else base / f"run_{stamp}"
        if not _run_complete(run_dir):
            raise FileNotFoundError(f"指定 OpenBMI S3（5070）run 不完整: {run_dir}")
        return run_dir
    runs = sorted(
    ...
    for r in runs:
        if _run_complete(r):
            return r
    raise FileNotFoundError(
        f"{model}: 无完整 OpenBMI S3 Acc_paper run under {base}"
    )
