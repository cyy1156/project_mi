"""ShallowFBCSPNet：加权CE + BalAcc 早停（特异度专项，位于 task_specificity/）。

相对旧 baseline_shallow.py：Task 用 w0/w1 加权 CE，早停盯 Balanced Acc；Three 逻辑不变。
用法：python baseline_shallow_wce2_balacc.py --data merged_2s
"""
...
def main() -> None:
    p = argparse.ArgumentParser(description="Shallow wce2+BalAcc（task_specificity）：Task+Three")
    ...
    append_md(
        ...
        "\n".join(
            [
                ...
                f"- model：`{MODEL_NAME}`（task_specificity；加权CE+BalAcc）",
