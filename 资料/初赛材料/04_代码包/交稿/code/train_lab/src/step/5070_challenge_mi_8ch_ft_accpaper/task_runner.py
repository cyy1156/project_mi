"""Exp34 轨 B 训练：8ch LOSO6 · OpenBMI 热启动可选 · 5070。"""
...
from openbmi_init import apply_init_weights, load_init_state  # noqa: E402
...
def train_one_fold(
    *,
    ...
    if n_chans != hp.n_chans_expected or n_times != hp.n_times_expected:
        raise RuntimeError(f"shape mismatch ch={n_chans} T={n_times}")

    mode = "OpenBMI-ft" if hp.init_from_openbmi else "scratch"
    print(
        f"\n======== [{model_name}/{mode}] fold{fold} ========\n"
        f"  train={fold_info['train_subjects']}\n"
        f"  val  ={fold_info['val_subjects']}\n"
        f"  n={len(tr_idx)}/{len(va_idx)} batch={hp.batch_train}",
        flush=True,
    )
    ...
    if hp.init_from_openbmi:
        state = load_init_state(model_name, remap_labels=hp.remap_openbmi_labels)
        msgs = apply_init_weights(model, state, strict=False)
        print(f"  init OpenBMI ok ({'; '.join(msgs) if msgs else 'strict-ish'})")
    ...
    for epoch in range(1, hp.max_epochs + 1):
        ...
        if acc > best_acc + 1e-6:
            ...
            torch.save(
                {
                    ...
                    "fold": fold,
                    "init_from_openbmi": hp.init_from_openbmi,
                    "hp": dict(hp.__dict__),
                },
                ckpt_path,
            )
            ...
            bad += 1
            if bad >= hp.patience:
                print(f"  early stop @ ep{epoch} best={best_acc:.4f}")
                break
...
def run_baseline_main(
    *,
    ...
    parser.add_argument("--scratch", action="store_true", help="B8-scratch：不用 OpenBMI 热启动")
    args, _ = parser.parse_known_args()
    if args.scratch:
        from shared_hparams import hp_scratch
    ...
    print(f"data X={tuple(X.shape)} from {data_dir} init={hp.init_from_openbmi}")

    tag = args.run_tag.strip() or datetime.now().strftime("%Y%m%d_%H%M%S")
    arm = "ft" if hp.init_from_openbmi else "scratch"
    run_dir = (
    ...
    print(
        f"[done] {model_name}/{arm} "
        f"{summary['val_acc_mean']:.4f}±{summary['val_acc_std']:.4f} → {run_dir}"
    )
    return run_dir
