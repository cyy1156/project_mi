...
# time: braindecode 时域（ArrayTask/Three 可 squeeze 通道维）
# feat: bandpower 等 (N,8,F)；raw: 已 squeeze 的原始时域 (N,8,T) — 二者共用 ArrayFeatDataset
_INPUT_KINDS_FEAT_LIKE = frozenset({"feat", "raw"})
_INPUT_KINDS = frozenset({"time", "feat", "raw"})
...
def _uses_feat_dataset(input_kind: str) -> bool:
    return input_kind in _INPUT_KINDS_FEAT_LIKE


def _fill_single_dataset_by_prefix(by_ds: dict, test_subjects: np.ndarray) -> dict:
    """单库被试 ID 无 ``bci2a:`` / ``stieger:`` 前缀时，用 overall 填对应 *_only。"""
    subs = [str(s) for s in np.asarray(test_subjects)]
    if not subs or any(":" in s for s in subs):
        return by_ds
    # 无前缀：本包仅跑 BCI2a，overall ≡ bci2a_only
    if by_ds.get("bci2a_only") is None:
        by_ds = dict(by_ds)
        by_ds["bci2a_only"] = dict(by_ds["overall"])
    return by_ds
...
def train_task_one_fold(
    ...
    *,
    ...
    def make_ds(mask):
        if _uses_feat_dataset(input_kind):
    ...
    for ep in range(1, hp.max_epochs + 1):
        ...
        if score > best_score:
            ...
            torch.save(
                {
                    "stage": f"task2_{model_name}_fixed2s",
                    "fold": fold,
    ...
    by_ds = _fill_single_dataset_by_prefix(
        metrics_by_dataset_prefix(
    ...
    *,
    ...
    for ep in range(1, hp.max_epochs + 1):
        ...
        if score > best_score:
            ...
            torch.save(
                {
                    "stage": f"three3_{model_name}_fixed2s",
                    "fold": fold,
                    "model_name": model_name,
                    "n_outputs": 3,
                    "protocol": hp.protocol,
...
def run_baseline_main(
    *,
    model_name: str,
    build_model: BuildFn,
    input_kind: str = "time",  # time | feat | raw
    structure_note: str,
    extra_meta: dict | None = None,
    ...
    if input_kind not in _INPUT_KINDS:
        raise ValueError(f"input_kind={input_kind!r}；可选: {sorted(_INPUT_KINDS)}")

    p = argparse.ArgumentParser(
        description=f"{model_name} fixed2s 离线基线（Task+Three；BalAcc+balbatch，无RAP）"
    )
    p.add_argument(
        "--data",
        default=default_data or SHARED.data_tag,
        choices=("bci2a_2s",),
        help="仅 BCI2a 固定窗 tag；不做 merged / Stieger",
    )
    p.add_argument(
        "--skip-three",
        action="store_true",
    ...
    out_name = f"{model_name}_fixed2s_balbatch_balacc"
    out_root = TRAIN_LAB / "out" / "baseline_fixed_2s" / out_name / data_tag / f"run_{stamp}"
    ...
    append_md(
        ...
        "\n".join(
            [
                ...
                f"- device：`{device}`",
                f"- data：`{data_dir}`（prefix=`{prefix}`，**仅 BCI2a；不合并**）",
                f"- protocol：`{hp.protocol}` | **no_rap=True**",
                f"- 读数口径：`Tw=2s fixed cue+2_4s rest_cue_before_2s bci2a_only balacc+balbatch no_rap`",
                f"- 切窗：Task=Cue+2~4s；Rest=Cue前2s；**固定窗无 hop**",
