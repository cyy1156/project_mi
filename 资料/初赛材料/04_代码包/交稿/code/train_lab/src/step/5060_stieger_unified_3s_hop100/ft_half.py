"""S10-02 / S10-02b：OpenBMI init · Stieger cue 前半 FT · 后半评。"""
...
RESULTS = results_ft_half_dir()
RESULTS_05 = RESULTS  # S10-02/05 同协议
...
def main() -> None:
    p = argparse.ArgumentParser(description="S10-02/02b Stieger 前半 FT")
    p.add_argument("--tw", default="3s", choices=("2s", "3s"), help="窗长：3s 或 2s")
    ...
    md = [
        f"# {prof.arm_ft_half} Stieger 前半 FT · {stamp}",
        "",
        f"- init：`{init_run}`",
    ...
    for stream in iter_subject_streams(subjects=subjects):
        ...
        write_split_artifacts(stream, split, DOCS_10 / "out" / stream.subject_id)
        log_line(
            log_path,
    ...
    note = (
        f"# {prof.arm_ft_half} FT 记录\n\n"
        f"主记录见：`{result_root}`\n"
        f"权重：`{weight_root}`\n"
    )
    (RESULTS / f"{stamp}_report_link.md").write_text(note, encoding="utf-8")
