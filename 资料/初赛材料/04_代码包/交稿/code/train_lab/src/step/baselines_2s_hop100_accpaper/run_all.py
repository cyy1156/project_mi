"""串跑十一模型 Acc_paper 重训（仅 bci2a_2s_hop100）。

用法（在 baselines_2s_hop100_accpaper/ 目录下）：
  python run_all.py
  python run_all.py --models eegnet,shallow --continue-on-error
  python run_all.py --smoke
  python baseline_eegnet.py --skip-three --max-folds 1 --max-epochs 2 --patience 2
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="串跑 baselines_2s_hop100_accpaper")
    p.add_argument(
        "--models",
...
def run_one(name: str, extra: list[str], continue_on_error: bool) -> int:
    ...
    if not script.is_file():
        ...
        return 1
    cmd = [sys.executable, str(script), "--data", "bci2a_2s_hop100", *extra]
    print(f"\n===== accpaper / {name} =====", flush=True)
