"""数据加载 / 折划分 / Acc_paper·BalAcc 评测共用（BCI2a 4s）。"""
...
def load_bci2a(hp: SharedHP = SHARED):
    """加载 bci2a_4s；无 trial_id 文件时用 arange（一窗一试次）。"""
    ...
    tid_path = data_dir / f"{prefix}_trial_id.npy"
    if tid_path.is_file():
        trial_ids = np.load(tid_path)
    else:
        trial_ids = np.arange(int(X.shape[0]), dtype=np.int64)
    assert int(X.shape[-1]) == hp.n_times_expected, (
        f"期望 T={hp.n_times_expected}，得到 {X.shape[-1]}"
    )
    assert int(X.shape[-2]) == hp.n_chans
    return {
        "X": X,
        "y_three": y_three,
        "subjects": subjects,
        ...
        "x_path": str(x_npy),
        "data_dir": data_dir,
        "trial_id_synthetic": not tid_path.is_file(),
    }


# 兼容旧名（脚本内若误写）
load_openbmi = load_bci2a
...
def materialize_windows(X, indices, y=None):
    """把选中窗拷进 RAM，避免 mmap 逐条 .copy() 饿死 GPU。"""
    idx = np.asarray(indices, dtype=np.int64).reshape(-1)
    X_ram = np.asarray(X[idx], dtype=np.float32)
    y_ram = None if y is None else np.asarray(y, dtype=np.int64)[idx]
    local = np.arange(len(idx), dtype=np.int64)
    return WindowDataset(X_ram, local, y=y_ram, x_path=None)
...
def build_backbone(hp: SharedHP) -> nn.Module:
    from lejepa_model import EEGLeJEPA
    ...
    return EEGLeJEPA(
        ...
        lambda_sigreg=hp.lambda_sigreg,
        num_slices=hp.num_slices,
    )
