"""单臂入口 · 5070 掩码未来双专家（本机正式 · 低内存 · 默认 fold0）.
...
def _run_a0_ref(argv_rest: list[str]) -> None:
    ...
    sys.modules.pop("_mfde5070_official_task_runner", None)
    ...
    tr.run_baseline_main(
        ...
        input_kind="time",
        structure_note="A0-ref braindecode Shallow · 500pt · Three-only · Acc_paper（量级参考 · 5070）",
        extra_meta={"arm": "A0_ref", "package": OUT_ROOT_TAG, "heads": "three-only"},
    )


def main() -> None:
    p = argparse.ArgumentParser(description="5070 mask-future dual-expert arm (primary low-mem)")
