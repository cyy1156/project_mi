"""试次级聚合：论文 Acc（正确窗占比 >50%）+ 众数 BalAcc/F1。"""
...
def aggregate_windows_to_trials(
    ...
    *,
    ...
    """
    按 (subject, trial_id) 聚合。

    Returns dict with trial-level arrays and metrics.
    """
    ...
    for subj, tid in order:
        ...
        if maj == TIE_SENTINEL:
            # 并列 → 计错：强制预测为错误类，便于 Spec/Rec/BalAcc
    ...
    # majority Acc：pred==true（并列已强制为错类）
...
def unit_check_boundary() -> None:
    """>50% 边界与并列众数。"""
    # 2/4 = 0.5 → paper fail；众数对 → majority ok
    y = np.array([1, 1, 1, 1])
    p = np.array([1, 1, 0, 0])
    ...
    assert r["metrics"]["acc_majority"] == 0.0  # tie 2-2 → forced wrong

    # 3/4 > 0.5 → paper ok；众数 1
    p2 = np.array([1, 1, 1, 0])
    ...
    # three-class: majority class != paper when plurality <50% correct
    y3 = np.array([1, 1, 1, 1, 1])
    p3 = np.array([1, 1, 2, 2, 0])  # correct 2/5=0.4；众数并列 1 与 2
    r3 = aggregate_windows_to_trials(
        y3, p3, np.array(["A"] * 5), np.array([1] * 5), n_classes=3
    )
    assert r3["metrics"]["acc_paper"] == 0.0
    # 1 和 2 各 2 票并列 → forced wrong
    assert r3["metrics"]["acc_majority"] == 0.0

    print("trial_aggregate unit_check OK")
