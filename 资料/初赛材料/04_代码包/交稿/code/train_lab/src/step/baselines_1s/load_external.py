...
def load_sibling_module(unique_name: str, script: Path, *, prefer_dir: Path | None = None) -> ModuleType:
    ...
    加载结束后若污染了 ``shared_hparams`` / ``md_fold_detail``，恢复为 baselines_1s 版本，
    ...
    try:
        ...
        if inserted is not None and inserted in sys.path:
            sys.path.remove(inserted)
        # 恢复 baselines_1s 侧模块；必要时重新从 HERE 加载
        here = str(HERE)
        if here in sys.path:
            sys.path.remove(here)
        sys.path.insert(0, here)
        for k, prev in saved.items():
            if prev is not None and "/baselines_1s/" in getattr(prev, "__file__", "").replace("\\", "/"):
                sys.modules[k] = prev
            else:
                sys.modules.pop(k, None)
                try:
