"""全模型微调：Acc_paper init → 前半训 → 后半伪在线评。"""
...
def run_ft_main(
    *,
    ...
    p = argparse.ArgumentParser(
        description=f"{model_name} 游戏全模型微调（前半训/后半评）"
    )
    ...
    p.add_argument("--run-stamp", default="", help="指定 accpaper init run")
    ...
    init_run = resolve_accpaper_run(
        model_name, run_stamp=args.run_stamp or None
    )
    print(f"[init] {model_name} Acc_paper: {init_run}", flush=True)

    stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    out_name = f"{model_name}_game_ft_half"
    result_root = DOCS_OUT / "results" / f"{stamp}_{out_name}"
    result_root.mkdir(parents=True, exist_ok=True)
    weight_root = (
        FT_WEIGHT_ROOT
        / f"{model_name}_game_ft_half_balbatch_accpaper"
        / stamp
    )
