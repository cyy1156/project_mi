# Scheme 17 路 external memory watchdog (backup for in-process mem_guard).
# Kills the python process tree if commit/WS/sys-free explode.
#
# 榛樿寮瑰嚭鍙鎺у埗鍙扮獥鍙ｏ紙Tee 鍚屾鍐欐棩蹇楋級锛涘悗鍙伴潤榛樺姞 -NoConsole銆?param(
  [Parameter(Mandatory = $true)][string]$Arm,
  [string]$ExtraArgs = "--max-folds 0 --num-workers 0",
  [double]$MaxProcVirtGB = 0,   # 0 = auto锛?090 榛樿楂樻。锛屽嬁鐢?5060 鐨?40G锛?  [double]$MaxProcWsGB = 0,     # 0 = auto锛?090 鍕跨敤 5060 鐨?14G锛?  [double]$MinSysFreeGB = 0.20,
...
# 浠撳簱鏍?= .../step/pkg 鈫?涓?5 绾у埌 MI锛堜笌 run_chain_detached.bat 涓€鑷达級
$RepoRoot = (Resolve-Path (Join-Path $WorkDir "..\..\..\..\..")).Path
$TrainLab = (Resolve-Path (Join-Path $WorkDir "..\..\..")).Path
# 5090锛氫紭鍏?PATH 涓婄殑 python锛坈onda activate cyy锛夛紝鍐嶅洖閫€浠撳簱 .venv
$py = $null
...
$existing = @(Get-CimInstance Win32_Process -Filter "Name='python.exe'" -EA SilentlyContinue |
  Where-Object {
    $_.CommandLine -and $_.CommandLine -match '5070_mask_future_dual_expert_accpaper\\run_arm\.py'
  })
...
$runnerBody = @"
`$ErrorActionPreference = 'Continue'
try { `$Host.UI.RawUI.WindowTitle = 'scheme17 路 $Arm 路 mem-guarded' } catch {}
...
# 2>&1 鍦?PS5 浼氭妸 stderr 鍙樻垚 ErrorRecord锛堢孩瀛?NativeCommandError锛夛紱缁熶竴杞垚鏅€氳鍐?Tee
