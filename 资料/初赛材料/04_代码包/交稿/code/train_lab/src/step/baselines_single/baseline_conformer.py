"""EEGConformer 单模型入口：Task + Three 独立五折，写 MD。不用 registry。"""
...
def run_task_kfold(X, y, subjects, device, hp: SharedTrainHP, out_dir: Path, data_tag: str) -> dict:
    ...
    summary = {
        ...
        "data_tag": data_tag,
        "hparams": shared_as_dict(),
        "conformer": {"backbone": "EEGConformer", "num_layers": CONFORMER_NUM_LAYERS, "num_heads": CONFORMER_NUM_HEADS, "att_drop_prob": CONFORMER_ATT_DROP},
        "val_f1_mean": vm,
        "val_f1_std": vs,
        "test_f1_mean": tm,
        "test_f1_std": ts,
...
def main() -> None:
    p = argparse.ArgumentParser(description="EEGConformer 单模型：Task+Three 独立五折")
    ...
    append_md(
        ...
        "\n".join(
            [
                ...
                f"- 结构：EEGConformer（num_layers=2, num_heads=10, att_drop=0.5 + shared drop_prob）",
