...
每个脚本 `--help` 可查全部参数；切窗参数与 `config/*.yaml` 一一对应（窗口、通道、滤波、标签映射都在里面）。产物写入 `code/preprocess_lab/out/<数据集_口径>/`（如 `openbmi_3s_hop100/`）。

### 第 3 步 · 训练与评测

实验入口都在各自目录内，典型结构是「训练脚本 → 产物落 out/ → 评测/复算脚本读 out/」。关键复算入口（对应《离线性能验证报告》sheet 05 原始数据索引）：
...
- **显卡不同能否复现**：指标以随机种子固定 + 验证集早停控制；不同 CUDA 版本可能在 ±0.5 个百分点内浮动，属正常。
