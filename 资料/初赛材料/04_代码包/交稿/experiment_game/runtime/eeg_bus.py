...
# 总册阈值：>2s L2 告警；>5s L3 中止（与 protocol.yaml eeg_watchdog 一致）
DEFAULT_STALL_S = 2.0
DEFAULT_LOST_S = 5.0
...
def resolve_eeg_watchdog(*, root: Optional[Union[Path, str]] = None) -> Dict[str, float]:
    """读 protocol.yaml ``eeg_watchdog``；缺省回落到 DEFAULT_*。

    返回 ``stall_s`` / ``abort_s``（= lost_s）/ ``judge_buf_stale_s``。
    """
    stall = float(DEFAULT_STALL_S)
    abort = float(DEFAULT_LOST_S)
    judge_stale = float(DEFAULT_JUDGE_STALE_S)
    try:
        from experiment_game.core.config import load_layered_config

        cfg = load_layered_config(root=Path(root) if root is not None else None)
        wd = cfg.get("eeg_watchdog") if isinstance(cfg, dict) else None
        if isinstance(wd, dict):
            if wd.get("stall_s") is not None:
                stall = float(wd["stall_s"])
            if wd.get("abort_s") is not None:
                abort = float(wd["abort_s"])
            elif wd.get("lost_s") is not None:
                abort = float(wd["lost_s"])
            if wd.get("judge_buf_stale_s") is not None:
                judge_stale = float(wd["judge_buf_stale_s"])
    except Exception:  # noqa: BLE001
        pass
    return {
        "stall_s": max(0.1, stall),
        "abort_s": max(stall, abort),
        "judge_buf_stale_s": max(0.05, judge_stale),
    }
