...
> **【现行入口 · 2026-07 更新】**  
> - 正式五折头2：`code/train_lab/src/step/train_three_kfold.py`  
> - 过夜联跑：`run_overnight_kfold.py`（先头1 再迁权重训头2）  
> - 数据：`out/bci2a_2s`，`n_times=500`；环境：仓库根 `.venv`  
> 下文保留早期「从 `train_task.py` 迁权重到 `train_three.py`」教学示例，可作对照。
