...
| 训练包（本机 5060） | `code/train_lab/src/step/5060_baselines_openbmi_2s_hop100_accpaper/`（**11 模型**；patience=20） |
| 训练包（5090，勿在本机改） | `code/train_lab/src/step/5090_baselines_openbmi_2s_hop100_accpaper/` |
| 权重 | `train_lab/out/5060_baseline_openbmi_2s_hop100_accpaper/<model>_openbmi_2s_hop100_balbatch_accpaper/...` |
| 本目录汇总 | `实验结果汇总_5060_baselines_openbmi_2s_hop100_accpaper.md`（跑完后写） |
...
| **S3** | 十一模型 × Task→Three × 五折正式 | **已完成**（见结果副本 / 权重清单） |
| **S4** | 写汇总 MD + 与 BCI2a 对照表 | **已完成** → [`实验结果汇总_5060_baselines_openbmi_2s_hop100_accpaper.md`](./实验结果汇总_5060_baselines_openbmi_2s_hop100_accpaper.md) |
...
下一步：`5060_baselines_openbmi_2s_hop100_accpaper` 冒烟后 `run_all.py --continue-on-error`。
