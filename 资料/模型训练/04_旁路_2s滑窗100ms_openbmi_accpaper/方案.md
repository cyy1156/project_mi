# 方案：OpenBMI · 2s/hop100 · Acc_paper 选模重训（Task Top-8）

> 状态：**全量八模训练进行中** · 2026-08-05  
...
> **模型名单**：BCI2a 历史 trialmaj **Task Acc_paper 前八**（见 §4）；**不跑** `*_raw`。  
> **数据**：已合并 `preprocess_lab/out/openbmi_2s_hop100`，`X=(680400,1,8,500)`，54 被试。  
...
在 **OpenBMI / Lee2019-MI** 上，按与现行 03 **相同的 Acc_paper 选模协议**，对 **Task 试次级 Acc_paper 前八模型** 从零重训并五折汇报；切窗几何与 BCI2a hop100 对齐（\(T_w=2\,\mathrm{s}\)，hop=\(100\,\mathrm{ms}\)，`(N,1,8,500)`），验证协议在大样本外库上是否可迁移。
...
| 名单？ | 仅 Top-8（§4）；**不含** `dbn_raw` / `gcbnet_raw` / `dgcnn_raw` |
| 与 03 | 配方同形；**数据与权重目录独立** |
...
| 块用法 | **合并** `EEG_MI_train` ∪ `EEG_MI_test`（二者均有标签）；**不做**官方会话内 train/test 夺冠 |
| 类别 | 仅 left / right；无脚舌 |
...
| 指标 | 用途 |
|------|------|
| **Acc_paper** | Val 早停 + Test 主报 + 八模型选模 |
...
名单来自 Task Top-8，但 **八模型对 Task、Three 各训一套**；两套头各自出 Acc_paper 主表与选模（不混用 ckpt）。
...
## 3. 模型范围（Task Acc_paper Top-8 · 冻结）

名单来源（**仅借名单，不借分数**）：  
[`../03_旁路_2s滑窗100ms_试次多数票/实验结果汇总_baselines_2s_hop100_trialmaj_bci2a.md`](../03_旁路_2s滑窗100ms_试次多数票/实验结果汇总_baselines_2s_hop100_trialmaj_bci2a.md) §2 Task 表。
...
**明确不跑**：`dgcnn_raw` / `gcbnet_raw` / `dbn_raw`（Task 表第 9–11）。
...
| 训练包 | `code/train_lab/src/step/baselines_openbmi_2s_hop100_accpaper/`（8 模型；patience=20） |
...
选模（冻结）：同一头下，八模型比 **Test Acc_paper 五折均值**；并列先看 std，再看 BalAcc_maj。
...
- 每被试每会话：约 200 MI + 200 Rest 段 → ~8400 窗  
- ×2 会话 → ~1.7×10⁴ 窗/人  
- ×54 → 约 **9×10⁵** 窗（远大于 BCI2a T）

建议：P1 先 **3 被试冒烟** → P2 再全量；图网络（gcbnet/dgcnn/dbn）注意 bandpower 预计算缓存。
...
| **S1** | OpenBMI loader + `openbmi_2s_hop100` 预处理 | **代码已落地**；全量待跑 |
| **S2** | 训练包（8 模型） | **代码已落地**；待数据齐后冒烟 |
| **S3** | 八模型 × Task→Three × 五折正式 | 待开工 |
...
| 2 | patience | **20** |
| 3 | 头 | **Task 与 Three 都跑**（八模型 × 两头独立训） |

下一步：先跑全量（或子集）预处理 → 再 `baselines_openbmi_2s_hop100_accpaper` 冒烟/正式。

---

## 11. 一句话

**OpenBMI 54 人 × 两会话（同人合并）；2s/100ms；Rest=Cue 前 4s、MI=Cue 后 0–4s；八模型从零重训；Task+Three；patience=20；Acc_paper 早停与选模；协议对齐 03。**
