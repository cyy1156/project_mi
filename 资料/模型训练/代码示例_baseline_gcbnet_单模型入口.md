...
- [ ] 已粘贴为 `baselines_single/baseline_gcbnet.py`（仓库此前**无**此 `.py`）
...
> 按本文手写落地 `baseline_gcbnet.py`：在线 5 频带 log 功率特征 → GCBNet → Task/Three 独立五折；复用 `shared_hparams`，不迁权重，不走 registry。
