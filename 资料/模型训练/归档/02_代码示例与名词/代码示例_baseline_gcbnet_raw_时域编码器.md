# 代码示例：GCBNet 原始时域 + TemporalEncoder（完整可粘贴）

> 性质：**完整示例文档**（格式对齐 [`代码示例_baseline_gcbnet_单模型入口.md`](./代码示例_baseline_gcbnet_单模型入口.md)）  
> 同步草稿：[`TepmoralEncoder_GCBNet.py`](../../code/train_lab/src/step/Self_development_model/TepmoralEncoder_GCBNet.py)  
> 对照 bandpower：[`代码示例_baseline_gcbnet_单模型入口.md`](./代码示例_baseline_gcbnet_单模型入口.md) / [`baseline_gcbnet.py`](../../code/train_lab/src/step/baselines_single/baseline_gcbnet.py)  
> **不要覆盖**现网 `baseline_gcbnet.py`；本方案 `MODEL_NAME = "gcbnet_raw"`
...
| 项 | 约定 |
|----|------|
| 入口 | `Self_development_model/TepmoralEncoder_GCBNet.py`（可再迁到 `baselines_single/baseline_gcbnet_raw.py`） |
| 输入 | **原始时域** `(N,8,500)`；**不做** `raw_to_bandpower` |
| 前端 | `TemporalEncoder`：`(B,8,500)→(B,8,D)`，默认 `D=NODE_DIM=64` |
| 图网络 | 与现网 GCBNet 同逻辑；仅 `in_channels=D` |
| 激活 | 默认 `relu_is=1`（B1ReLU）；保留 B2ReLU 备选 |
| Dataset | `ArrayFeatDataset`，此处 F=500 |
| 超参 | 复用 `baselines_single/shared_hparams.py` |
| Task / Three | 独立五折；无 weight_transfer；native 头 |
| 锁种 | 与现基线相同 |
| MD | `资料/模型训练/runs/{stamp}_gcbnet_raw/gcbnet_raw五折实验记录.md` |
| 权重 | `code/train_lab/out/baseline/gcbnet_raw/<data>/run_<stamp>/` |

数据流：

```text
(N,1,8,500) → squeeze_raw → (N,8,500) → batch (B,8,500)
  → TemporalEncoder → (B,8,D) → GCBNet(in_channels=D) → logits
```

相对旧 bandpower 草稿已落地修正（**本节 MD 的 §2 与 `.py` 全文一致**）：

1. `main` 用 `squeeze_raw`，去掉 `raw_to_bandpower` / `BANDS_HZ`  
2. `build_model_raw(n_outputs=..., drop_prob=...)`，不再误传 `(8, n_feats, ...)`  
3. `MODEL_NAME="gcbnet_raw"`；`input=raw_temporal_encoder`  
4. `sys.path` 加入 `baselines_single`（否则找不到 `shared_hparams` / `md_fold_detail`）  
5. 补回 `B2ReLU` + `relu_is`（默认仍 B1）

运行：

```text
cd code/train_lab/src/step/Self_development_model
python TepmoralEncoder_GCBNet.py --data merged_2s
```

说明：§2 代码块与 `TepmoralEncoder_GCBNet.py` **逐行同步**；改一处请同时改另一处。

---

## 1. `shared_hparams.py`（复用，不粘贴）
...
| 位置 | bandpower `baseline_gcbnet.py` | 本文 `gcbnet_raw` |
|------|-------------------------------|-------------------|
| `main` 特征 | `raw_to_bandpower` → `(N,8,2)` | `squeeze_raw` → `(N,8,500)` |
| 建模型 | `build_model(8, n_feats=2, ...)` | `build_model_raw(n_outputs, drop_prob, node_dim=64)` |
| 前向 | 直接 GCBNet | Encoder → GCBNet |
| `MODEL_NAME` | `gcbnet` | `gcbnet_raw` |

---

> 本文 §2 与 `TepmoralEncoder_GCBNet.py` 全文同步；以该 `.py` 为实际跑实验入口。
