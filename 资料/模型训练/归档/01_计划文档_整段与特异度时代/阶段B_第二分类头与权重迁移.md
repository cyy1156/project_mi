...
# 【历史文档】阶段 B：第二分类头 + 保存/加载第一头权重

> **【HISTORICAL / 勿作新默认 · 2026-07-28】**  
> **现行训练策略**：二分类与三分类**独立训练、不迁权重、原生分类头**。  
> → [`资料/实验结果说明/训练策略_二分类与三分类独立训练.md`](../实验结果说明/训练策略_二分类与三分类独立训练.md)  
> → 项目计划 §3.1.3  
>  
> **本文全文为历史「迁主干训三分类」教学与对照**；正文中的「本期推荐」均指**旧实现**。  
> 旧五折/过夜脚本已归入 `code/train_lab/src/step/归档_旧训练入口/`；新入口为 `baselines_single/`。  
> 以新策略文档与 [`归类清单_单模型入口重构.md`](./归类清单_单模型入口重构.md) 为准。

> **性质**：历史示例文档。  
> **正式报数划分**：见 `资料/模型训练/正式评估协议_被试独立五折.md`。
