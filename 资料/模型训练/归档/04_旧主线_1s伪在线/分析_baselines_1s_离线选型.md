# 【已废止】分析：`baselines_single` → `baselines_1s`（原结构、不做 RAP）

> **【已废止 · 2026-08-04】** 现行训练入口为 `baselines_2s_hop100/`（见 [`../../00_当前主线_2s滑窗100ms/`](../../00_当前主线_2s滑窗100ms/)）。  
> `baselines_1s/` **冻结只读**，不再扩 Stieger / 不再当主线。  
> ~~状态：代码已落地。~~ 历史策略：1 s 八基线；`bci2a_1s`/`stieger_1s`；Val BalAcc + batch balance；无 RAP。  
> 依据（旧）：[`方案_滑窗输入与RAP改造.md`](./方案_滑窗输入与RAP改造.md)。
