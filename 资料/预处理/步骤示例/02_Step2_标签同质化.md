...
> 依据项目计划 **§3.1.3 双标签与训练策略**：  
> 每个 EEG 样本**不复制两份**，在同一条记录上同时保存两套监督标签；二分类与三分类**独立训练、不迁权重**。

## 目标

把各数据集的原始标签，统一成**双标签**，供后续**两套独立任务**使用：
| 标签 | 含义 | 取值 |
|------|------|------|
| `label_task` | 任务状态（二分类任务：静息态 / 任务态） | `0`=静息，`1`=任务 |
| `label_three` | 三分类任务：空闲 / 左手 / 右手 | `0`=空闲，`1`=左手，`2`=右手 |
