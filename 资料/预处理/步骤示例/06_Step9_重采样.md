...
|------------|---------|-------|
| **BCI2a 现行** | **2.0** | **500** |
| Stieger 等（若仍取 4s） | 4.0 | 1000 |

函数名历史原因仍叫 `resample_to_1000`，实际由 `win_sec` 决定点数。
...
# 4s 库仍可传 win_sec=4.0 → 1000
e = np.random.randn(1000, 8)
f = resample_to_1000(e, 250.0, win_sec=4.0)
...
- 2a 流水线忘传 `win_sec=2.0`，默认旧值 4.0 会得到错误长度。
